...
function ReturnDialog({ open, issue, onClose, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      await sportsService.returnIssue(issue.uuid, returnData);
      onSuccess();
      onClose();
    } catch (err) {
...
export default function SportIssueList() {
  ...
  const loadIssuesWithFilters = async (overrideFilters = {}) => {
    ...
    try {
      ...
      const data = await sportsService.getIssues(filters);
      setIssues(data);
    } catch (err) {
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await sportsService.deleteIssue(deleteDialog.item.uuid);
