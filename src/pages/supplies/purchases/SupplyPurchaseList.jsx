...
export default function SupplyPurchaseList() {
  ...
  const handleClear = () => {
    ...
    setBatches([]);
    setLoading(true);
    suppliesService.getPurchaseBatches({}).then((data) => { setBatches(data); setPage(0); })
      .catch(() => setError('Failed to load purchases')).finally(() => setLoading(false));
  };

  const toggleExpand = async (uuid) => {
    const next = new Set(expandedRows);
    if (next.has(uuid)) { next.delete(uuid); setExpandedRows(next); return; }
    next.add(uuid);
  ...
  const expandedItemSx = {
    ...
    '& td:first-of-type': { borderLeft: '3px solid', borderColor: 'primary.light' },
  };
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => navigate('/supplies/purchases/add')}>Add Purchase</Button>
      </Box>
      ...
      <Card>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', px: 2, pt: 1 }}>
          <FormControlLabel
            control={<Checkbox checked={showDetails} onChange={(e) => setShowDetails(e.target.checked)} size="small" />}
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ '& th': { fontWeight: 600, whiteSpace: 'nowrap' } }}>
                ...
                <TableCell align="center" sx={{ width: 56 }}>Bill</TableCell>
            ...
            <TableBody>
              {!loading && paginatedBatches.length === 0 && (
                <TableRow>
                  <TableCell colSpan={colCount} align="center" sx={{ py: 4, color: 'text.secondary' }}>No purchases found</TableCell>
                </TableRow>
              )}
                ...
                return (
                  <React.Fragment key={row.uuid}>
                    <TableRow sx={isDeleted ? deletedRowSx : undefined} hover={!isDeleted}>
                      ...
                      <TableCell>
                        <Chip label={`${row.itemCount ?? 0} item${(row.itemCount ?? 0) === 1 ? '' : 's'}`} size="small" color="primary" variant="outlined" onClick={() => toggleExpand(row.uuid)} sx={{ cursor: 'pointer' }} />
                      </TableCell>
                      <TableCell align="right" sx={{ whiteSpace: 'nowrap' }}>{row.totalCost != null ? parseFloat(row.totalCost).toFixed(2) : '—'}</TableCell>
                      <TableCell>{row.supplier || '—'}</TableCell>
                      <TableCell align="center">
                        {row.fileId && (
                          <Tooltip title="View bill">
                            <IconButton size="small" color="primary" onClick={() => openBill(row.uuid)}><AttachFileIcon fontSize="small" /></IconButton>
                          </Tooltip>
                        )}
                      </TableCell>
                      ...
                      <TableCell align="center" sx={{ whiteSpace: 'nowrap' }}>
                        {!isDeleted && isGod && (
                          <Tooltip title="Edit">
                            <IconButton size="small" color="primary" onClick={() => navigate(`/supplies/purchases/${row.uuid}/edit`)}><EditIcon fontSize="small" /></IconButton>
                          </Tooltip>
                        )}
                        {!isDeleted && (
                          <Tooltip title="Delete">
                            <IconButton size="small" color="error" onClick={() => setDeleteDialog({ open: true, item: row })}><DeleteIcon fontSize="small" /></IconButton>
                          </Tooltip>
                        )}
                        {isDeleted && isGod && (
                          <Tooltip title="Restore deletion">
                            <IconButton size="small" color="primary" onClick={() => setRestoreDialog({ open: true, item: row })}><RestoreIcon fontSize="small" /></IconButton>
                          </Tooltip>
                        )}
                      </TableCell>
                    </TableRow>
                    ...
                    {isExpandLoading && (
                      <TableRow key={row.uuid + '_loading'}>
                        <TableCell colSpan={colCount} sx={{ py: 0, px: 0 }}><LinearProgress /></TableCell>
                      </TableRow>
                    )}

                    {isExpanded && !isExpandLoading && (batchItemsCache[row.uuid] || []).map((item) => (
                      <TableRow key={item.uuid} sx={expandedItemSx}>
                        <TableCell sx={{ pr: 0 }} />
                        <TableCell />
                        <TableCell><Typography variant="body2">{item.itemName || item.itemId}</Typography></TableCell>
                        <TableCell align="right" sx={{ whiteSpace: 'nowrap' }}>
                          {item.costPerUnit ? (item.quantity * parseFloat(item.costPerUnit)).toFixed(2) : '—'}
                        </TableCell>
                        <TableCell>Qty: {item.quantity} {item.costPerUnit ? `@ ${formatCurrency(item.costPerUnit)}` : ''}</TableCell>
                        <TableCell />
                        {showDetails && <TableCell>{row.invoiceNumber || '—'}</TableCell>}
      ...
      <Dialog open={billDialog.open} onClose={closeBill} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers sx={{ minHeight: 400, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {billDialog.url && (
            billDialog.mimeType === 'application/pdf' ? (
              <Box component="iframe" src={billDialog.url} title="bill" sx={{ width: '100%', height: 560, border: 'none' }} />
            ) : (
              <Box component="img" src={billDialog.url} alt="bill" sx={{ maxWidth: '100%', maxHeight: 560, objectFit: 'contain' }} />
            )
          )}
        </DialogContent>
        <DialogActions>
          {billDialog.url && <Button component="a" href={billDialog.url} download={billDialog.fileName} size="small">Download</Button>}
