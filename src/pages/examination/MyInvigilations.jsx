...
export default function MyInvigilations() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [d, s] = await Promise.all([examinationService.myInvigilations(), examinationService.getMySignature()]);
      setDuties(d); setSig(s);
    } catch (e) {
    ...
    <Box sx={{ maxWidth: 720, mx: 'auto' }}>
      ...
      <Card>
        <CardContent>
          ...
          {duties.length === 0 ? (
            <Typography color="text.secondary" sx={{ py: 2 }}>No invigilation duties assigned in any published exam.</Typography>
          ) : duties.map((d, i) => (
