...
import {
  Box, Typography, Button, Card, IconButton, Alert, Dialog, DialogTitle, DialogContent,
...
function ConfigDialog({ open, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    if (!name.trim() || !academicYearId) { setError('Name and academic year are required'); return; }
    setSaving(true);
    setError('');
    try {
      await timetableService.createConfig({ name: name.trim(), academicYearId });
      onSaved();
      onClose();
    } catch (err) {
      setError(err.response?.data?.error?.description || 'Failed to create config');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        ...
        <TextField autoFocus fullWidth label="Config Name" value={name}
          onChange={(e) => setName(e.target.value)} sx={{ mt: 1, mb: 2 }} />
        <AcademicYearSelect value={academicYearId} onChange={setAcademicYearId} size="medium" sx={{ width: '100%' }} />
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose} disabled={saving}>Cancel</Button>
        <Button onClick={handleSave} variant="contained" disabled={saving}>{saving ? 'Saving...' : 'Create'}</Button>
      </DialogActions>
    </Dialog>
  );
}
...
export default function ConfigList() {
  ...
  const [archiveDialog, setArchiveDialog] = useState({ open: false, config: null });
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error?.description || 'Failed to archive config');
      setArchiveDialog({ open: false, config: null });
    } finally {
      setArchiving(false);
    }
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)}>New Config</Button>
      </Box>
      ...
      <Card>
        <DataGrid
          rows={configs} columns={columns} getRowId={(row) => row.uuid} loading={loading}
          autoHeight pageSizeOptions={[10, 25, 50]} initialState={{ pagination: { paginationModel: { pageSize: 25 } } }}
          disableRowSelectionOnClick onRowClick={(params) => navigate(`/timetable/configs/${params.row.uuid}`)}
          sx={{ border: 'none', cursor: 'pointer', '& .MuiDataGrid-columnHeaderTitle': { fontWeight: 600 }, '& .MuiDataGrid-cell': { borderBottom: '1px solid #e4e9f2' } }}
        />
      </Card>

      <ConfigDialog open={dialogOpen} onClose={() => setDialogOpen(false)} onSaved={load} />
      <ConfirmDialog
        open={archiveDialog.open} title="Archive Config" confirmLabel="Archive" loadingLabel="Archiving..."
        message={`Archive "${archiveDialog.config?.name}"? It will no longer be the active grid.`}
        onConfirm={handleArchive} onCancel={() => setArchiveDialog({ open: false, config: null })} loading={archiving}
      />
    </Box>
  );
}
