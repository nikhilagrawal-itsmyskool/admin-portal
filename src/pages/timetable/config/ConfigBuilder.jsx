  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        ...
        >
          {days
            .filter((d) => !existingDows.includes(d.value))
            .map((d) => (
        ...
        <Button
          ...
          onClick={() => {
            ...
            if (!Number.isInteger(seq)) {
              setError("Sequence must be an integer");
              return;
            }
            onSave({
              sequence: seq,
...
export default function ConfigBuilder() {
  ...
  const labelForDow = (dow) =>
    daysLookup.find((d) => d.value === dow)?.label || `Day ${dow}`;

  const addDay = async (data) => {
    try {
      await timetableService.createDay(id, data);
      setDayDialog(false);
      load();
    } catch (err) {
      setError(err.response?.data?.error?.description || "Failed to add day");
    }
  };
  const saveSlot = async (data) => {
    try {
      if (slotDialog.slot)
        await timetableService.updateSlot(slotDialog.slot.uuid, data);
      else await timetableService.createSlot(slotDialog.day.uuid, data);
      setSlotDialog({ open: false, day: null, slot: null });
      load();
    } catch (err) {
      setError(err.response?.data?.error?.description || "Failed to save slot");
    }
  };
  const doLock = async () => {
    setWorking(true);
    setError("");
    try {
      await timetableService.lockConfig(id);
      await load();
    } catch (err) {
      setError(err.response?.data?.error?.description || "Failed to lock");
    } finally {
      setWorking(false);
    }
  };
  const doUnlock = async () => {
    ...
    setError("");
    try {
      await timetableService.unlockConfig(id);
      await load();
    } catch (err) {
      setError(err.response?.data?.error?.description || "Failed to unlock");
    } finally {
      setWorking(false);
    }
  };
  const doClone = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error?.description || "Failed to clone");
      setWorking(false);
    }
  };
  const doConfirm = async () => {
    ...
    try {
      if (confirm.kind === "day")
        await timetableService.deleteDay(confirm.target.uuid);
  ...
  // Structural edits require god (canMutate) AND an unlocked config.
  const canEdit = canMutate && !locked;
    ...
    <Box>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <IconButton onClick={() => navigate("/timetable/configs")}>
            <BackIcon />
          </IconButton>
      ...
      {locked && (
        <Alert severity="info" sx={{ mb: 3 }}>
          This config is locked, so it can be used to generate timetables. To
          change it, clone it into a new draft and edit that.
        </Alert>
      )}
      ...
      <Box sx={{ display: "flex", gap: 2, overflowX: "auto", pb: 2 }}>
        {(config.days || []).length === 0 && (
          <Typography sx={{ color: "#8f9bb3" }}>
            No days yet. Add a weekday to start building the grid.
          </Typography>
        )}
        {(config.days || []).map((day) => (
            ...
            <CardContent>
              ...
              >
                <Typography variant="h6">
                  {day.label || labelForDow(day.dayOfWeek)}
                </Typography>
                {canEdit && (
                  <Tooltip title="Delete day">
                    <IconButton
                      ...
                      color="error"
                      onClick={() =>
                        setConfirm({ open: true, kind: "day", target: day })
                      }
                    >
              ...
              <Stack spacing={1}>
                    ...
                    <Box>
                      ...
                      >
                        ...
                        <Chip
                          size="small"
                          label={slot.slotType}
                          color={SLOT_COLORS[slot.slotType] || "default"}
                          variant="outlined"
                        />
                      </Box>
                    ...
                    {canEdit && (
                      <Box>
                        <IconButton
                          size="small"
                          onClick={() =>
                            setSlotDialog({ open: true, day, slot })
                          }
                        >
                ...
                {canEdit && (
                  <Button
                    ...
                    startIcon={<AddIcon />}
                    onClick={() =>
                      setSlotDialog({ open: true, day, slot: null })
                    }
                  >
                    Add Slot
                  </Button>
      ...
      <SlotDialog
        ...
        defaultSequence={
          (slotDialog.day?.slots || []).reduce(
            (m, s) => Math.max(m, s.sequence),
            0,
          ) + 1
        }
      ...
      <ConfirmDialog
        open={confirm.open}
        title={confirm.kind === "day" ? "Delete Day" : "Delete Slot"}
        message={
          confirm.kind === "day"
            ? "Delete this day and all its slots?"
            : "Delete this slot?"
        }
        onConfirm={doConfirm}
        onCancel={() => setConfirm({ open: false, kind: null, target: null })}
        loading={working}
      />
    </Box>
  );
