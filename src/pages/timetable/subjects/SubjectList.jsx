...
import {
  ...
  DialogActions, TextField, MenuItem, Tooltip, Chip,
} from '@mui/material';
...
const KIND_COLORS = { academic: 'primary', games: 'success', library: 'info', activity: 'warning' };
...
function SubjectDialog({ open, subject, kinds, onClose, onSaved }) {
  ...
  useEffect(() => {
    setForm({ name: subject?.name || '', code: subject?.code || '', kind: subject?.kind || 'academic' });
    setError('');
  }, [subject, open]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        ...
        <TextField autoFocus fullWidth label="Name" value={form.name}
        ...
        <TextField select fullWidth label="Kind" value={form.kind}
...
export default function SubjectList() {
  ...
  const columns = [
    ...
    { field: 'code', headerName: 'Code', width: 140 },
    {
      ...
      renderCell: (params) => <Chip size="small" label={params.value} color={KIND_COLORS[params.value] || 'default'} variant="outlined" />,
    },
    {
      ...
      renderCell: (params) => (
        <Box>
          <Tooltip title="Edit">
            <IconButton size="small" onClick={() => setDialog({ open: true, subject: params.row })}><EditIcon fontSize="small" /></IconButton>
          </Tooltip>
          <Tooltip title="Delete">
            <IconButton size="small" color="error" onClick={() => setDeleteDialog({ open: true, subject: params.row })}><DeleteIcon fontSize="small" /></IconButton>
          </Tooltip>
        </Box>
      ),
    },
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        ...
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialog({ open: true, subject: null })}>Add Subject</Button>
      </Box>
      ...
      <Card>
        <DataGrid
          rows={subjects} columns={columns} getRowId={(row) => row.uuid} loading={loading}
      ...
      <SubjectDialog open={dialog.open} subject={dialog.subject} kinds={kinds}
        onClose={() => setDialog({ open: false, subject: null })} onSaved={load} />

      <ConfirmDialog
        open={deleteDialog.open} title="Delete Subject"
        message={`Delete "${deleteDialog.subject?.name}"? Subjects used in classes cannot be deleted.`}
        onConfirm={handleDelete} onCancel={() => setDeleteDialog({ open: false, subject: null })} loading={deleting}
      />
    </Box>
  );
}
